...
class ErpMaintenanceModel(Base):
    __tablename__ = "quarter_maintenance_completed"
    __table_args__ = (
        UniqueConstraint(
            'emp_cd', 'div_cd', 'comp_cd', 'maintenance_cd',
            name='employee_quarter_maintenance_unique_constraint'
        ),
        ForeignKeyConstraint(
        ...
        ForeignKeyConstraint(
            ['comp_cd', 'div_cd', 'emp_cd', 'under_maintenance_cd' ],
            ['erp_admin.quarter_under_maintenance.comp_cd', 
             'erp_admin.quarter_under_maintenance.div_cd', 
             'erp_admin.quarter_under_maintenance.emp_cd',
             'erp_admin.quarter_under_maintenance.under_maintenance_cd'],
            use_alter=True
        ),
    ...
    maintenance_cd = Column('maintenance_cd', Integer, primary_key=True, index=True)
    under_maintenance_cd = Column('under_maintenance_cd', Integer)
    quarter = Column('quarter', Text)
    building = Column('building', Text)
    to_date = Column('to_date', Date)
    from_date = Column('from_date', Date)
    maintenance_reason = Column('maintenance_reason', Text)

...
class ErpQuarterAllotmentModel(Base):
    __tablename__ = "quarter_allotment"
    __table_args__ = (
        UniqueConstraint(
            'emp_cd', 'div_cd', 'comp_cd', 'quarter_allotment_cd',
            name='employee_quarter_allotment_unique_constraint'
        ),
        ForeignKeyConstraint(
    ...
    quarter_allotment_cd = Column('quarter_allotment_cd', Integer, primary_key=True, index=True)
    quarter = Column('quarter', Text)
    allotment_date = Column('allotment_date', Date)
    allotment_letter_no = Column('allotment_letter_no', Date)

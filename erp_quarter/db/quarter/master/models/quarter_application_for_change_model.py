...
class QuarterApplicationChangeModel(Base):
    __tablename__ = "quarter_app_chg"
    __table_args__ = (
        UniqueConstraint(
            'emp_cd', 'div_cd', 'comp_cd', 'quarter_app_chg_cd',
            name='employee_quarter_application_change_unique_constraint'
        ),
        ForeignKeyConstraint(
    ...
    quarter_app_chg_cd = Column('quarter_app_chg_cd', Integer, primary_key=True, index=True)
    accomodation_preferred = Column('accom_pref', String, nullable=True)
    quarter_preferred = Column('quarter_pref', String, nullable=True)

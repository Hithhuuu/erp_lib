...
class ErpQuarterApplicationModel(Base):
    __tablename__ = "quarter_application"
    __table_args__ = (
        UniqueConstraint(
            'emp_cd', 'div_cd', 'comp_cd', 'quarter_app_cd',
            name='employee_quarter_application_unique_constraint'
        ),
        ForeignKeyConstraint(
    ...
    quarter_app_cd = Column('quarter_app_cd', Integer, primary_key=True, index=True)
    unit = Column('unit', Text, nullable=True)
    quarter_prefered = Column('quarter_prefered', Text)
    pay_authority = Column('pay_authority', Text)
    current_accommodation = Column('current_accommodation', Text)
    application_date = Column('application_date', Date)
    date_sk_mcm_cm = Column('date_sk_mcm_cm', Date)

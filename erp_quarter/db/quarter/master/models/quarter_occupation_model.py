...
class ErpQuarterOccupationModel(Base):
    __tablename__ = "quarter_occupation"
    __table_args__ = (
        UniqueConstraint(
            'emp_cd', 'div_cd', 'comp_cd', 'quarter_occupation_cd',
            name='employee_quarter_occupation_unique_constraint'
        ),
        ForeignKeyConstraint(
    ...
    quarter_occupation_cd = Column('quarter_occupation_cd', Integer, primary_key=True, index=True)
    quarter = Column('quarter', Text)
    occupied_vide = Column('occupied_vide', Text)
    occupied_date = Column('occupied_date', Date)
